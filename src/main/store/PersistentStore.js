...
class PersistentStore {
  ...
  // ─── Test mode: reset + seed ───────────────────────────────────────────────

  /**
   * Wipe the on-disk SQLite + legacy JSON files and re-initialise to an empty
   * schema. Used by `--reset-state` for deterministic test runs.
   * Settings reset (electron-store) is the caller's responsibility.
   */
  async reset() {
    if (this._flushTimer) { clearTimeout(this._flushTimer); this._flushTimer = null; }
    if (this._db) { try { this._db.close(); } catch {} this._db = null; }
    for (const p of [this._dbPath, this._metaPath, this._metaPath + '.migrated']) {
      try { if (fs.existsSync(p)) fs.unlinkSync(p); } catch {}
    }
    if (fs.existsSync(this._msgDir)) {
      try {
        for (const f of fs.readdirSync(this._msgDir)) {
          try { fs.unlinkSync(path.join(this._msgDir, f)); } catch {}
        }
      } catch {}
    }
    await this.init();
  ...
  /**
   * Load a JSON fixture and insert sessions, messages, memory, and feedback
   * rows. Returns counts and any `settings` block (caller applies that to
   * SettingsStore).
   *
   * Fixture shape:
   * {
   *   sessions: [{ id, title?, mode?, created_at?, updated_at?, pinned? }],
   *   messages: [{ id?, session_id, role, content, type?, images?, created_at? }],
   *   memory:   [{ content, category?, mode?, source? }],
   *   feedback: [{ user_message, assistant_response, rating, ... }],
   *   settings: { ...electron-store key-value pairs to apply }
   * }
   */
  async seedFromFile(filePath) {
    const raw = fs.readFileSync(filePath, 'utf8');
    const fixture = JSON.parse(raw);
    const counts = { sessions: 0, messages: 0, memory: 0, feedback: 0 };

    if (Array.isArray(fixture.sessions)) {
      const stmt = this._db.prepare(
        `INSERT OR REPLACE INTO sessions (id, title, mode, created_at, updated_at, pinned, summary, summary_up_to_index)
         VALUES (?,?,?,?,?,?,?,?)`
      );
    ...
    if (Array.isArray(fixture.messages)) {
      const stmt = this._db.prepare(
        `INSERT OR REPLACE INTO messages (id, session_id, role, content, type, images, created_at)
         VALUES (?,?,?,?,?,?,?)`
      );
    ...
    if (Array.isArray(fixture.memory)) {
      for (const m of fixture.memory) {
        this.addMemory(m.content, m.source || 'manual', m.category || 'fact', m.mode || 'any');
        counts.memory++;
      }
    }

    if (Array.isArray(fixture.feedback)) {
      for (const f of fixture.feedback) {
        this.addFeedback({
          sessionId:         f.session_id || null,
          userMessage:       f.user_message,
          assistantResponse: f.assistant_response,
          rating:            f.rating,
          correction:        f.correction || null,
          model:             f.model || null,
          agentId:           f.agent_id || 'friday',
          appMode:           f.app_mode || 'chat',
        });
        counts.feedback++;
      }
    }
    ...
    this._flushNow();
    return { ...counts, settings: fixture.settings || null };
  }
